...
class FeasibleRoute(object):    
    ...
    def __str__(self):
        ...
        if self.combo_ID is None:
            ...
            if self.optimal:
                m += "{space:22} (OPTIMAL COMBINATION IF WE USE 2 BUS)\n\n".format(space='')
...
def getNode2Node(requestsList, nodeA, nodeB, quest):
    ...
    if quest == 'Duration':
        distance = request.distanceGen(locA, locB)
        return request.durationGen(locA, locB, distance)
        #return request.getDuration(locA, locB)
    else:                                                     #quest == 'Distance':
        return request.distanceGen(locA, locB)
        #return request.getDistance(locA, locB)
        
def printRouteInList(route):
    for i in route:
        if type(i) is Node:
            print i,
    print "]"
...
def isPossible(newRoute, time, requestsList):        # checks if a route is possible
    ...
    if lengthOfRoute is 1: 
        if lastNode.constraint_type is 'A':                    # if this a route with only one node (the beginning of a possible route) 
            ...
            time = lastNode_req.minDepTime            #CHANGED
    else:        
...
#        dur = request.durationGen()
        dur = getNode2Node(requestsList, busPosition, lastNode, "Duration")        #CHANGED / NO GOOGLEMAPS
...
def calcRoutes(requestsList):
    ...
    while possibleRoutes:                                            # while possibleRoutes is not an empty list 
        ...
        for next_route in nextPossibleRoutes:                        # if the route is complete, add to result
            if len(next_route) == target_length:                    # -1 because we have an extra item, "time".
...
def printTimedRoute(routes, timeline):
    arrow = '=>'
    print "Found %d routes in total.\n" % (len(routes))
    for num, (route, times) in enumerate(zip(routes, timeline)):
        print "\n{0:20}--------> Possible route: {num:<2} <--------\n\n".format('', num=num + 1)
        for node, time in zip(route, times):            
            print "{0:30} {1:5} {2:3} {3:8}".format('',node, arrow, time)
            
def find_routes_km(requestsList, route):
    route_length = len(route)
    meters = 0
    # KM GENERATOR
    #base_to_first_node = request.kmGen()
    #return_to_base = request.kmGen()
...
def with_one_van(requests, feasible_routes, feas_index):
    ...
    if possRoutes:
        ...
        print "No routes found with 1 van"
        return (False, 0, 0)
...
def with_two_vans(requests, van1, van2, feasible_routes, feas_index, combo):
    ...
    if possRoutesVan1 and possRoutesVan2:
        ...
        print "no routes with 2"
        return (False, 0, 0, 0)
    
def write_speed(requestsList, total_speed, req_speed, calc_speed_1, calc_speed_2, feas_elimination, find_optimal_speed):
    f = open('Statistics-prun-nomem.txt', 'a')
    f.write('Number of Requests: %s\n\n' % (len(requestsList)))
    f.write("Total speed was %.5f\n" % (total_speed))
    f.write("Requests speed was %.5f\n" % (req_speed))
    f.write("Calculate routes speed with 1 van was %.5f\n" % (calc_speed_1))
    f.write("Calculate routes speed with 2 vans was %.5f\n" % (calc_speed_2))
    f.write("Eliminating combos speed was %.5f\n" % (feas_elimination))
    f.write("Finding optimal with AHP speed was %.5f\n" % (find_optimal_speed))
    f.write("--------------------------------------\n")
    f.close()
    
def write_hills(requestsList, km_best, delay, swaps):
    f = open('Hill Climbing.txt', 'a')
    f.write('Number of Requests: %s\n\n' % (len(requestsList)))
    f.write("Number of swaps: %d\n" % (swaps))
    f.write("Local Bests (km): %s\n" % (km_best))
    f.write("Local bests (delay) %s\n" % (delay))
    f.write("--------------------------------------\n")
    f.close()