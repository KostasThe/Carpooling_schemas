...
@memoize
def distanceGen(pointA, pointB):
    meters = random.choice([i for i in range(500, 10000)])
    return meters
    
@memoize
def durationGen(pointA, pointB, meters):
    mins = meters / 500  # 30 km/h -> 30.000/60 met/min -> 500 met/min
    return MyTime(0, mins)
...
def makeRequest():
    ...
    for reqNumber in range(numberOfReqs):                    # Use iteration to fill in each Request values
        ...
        distance = distanceGen(departure[0], arrival[0])                # RANDOM DISTANCE GENERATOR
        duration = durationGen(departure[0], arrival[0], distance)            # RANDOM MINUTES GEN BASED ON DISTANCE
        #duration = getDuration(departure[0], arrival[0])        #CHANGED
