    ...
    if buses is 1:
        ...
        for route in feasible_routes:
            if route.combo_ID is best_route:
                route.optimal = True
            if route.combo_ID is not best_route:
                route.optimal = False
    else:        
        # CHOOSE OPTIMAL BETWEEN USING 1 OR 2 BUSES:
        
        if with_one_found and with_two_found:            # an vroume efiktes kai me 1 kai me 2 van
            km = []
            delay = []
            route_number = []
            ...
            result = apply_AHP(km, delay, route_number, 0)
            best_result = result
            for route in feasible_routes:
...
def apply_AHP(routes_km, customers_delay, combo_ID, buses):
    ...
    if buses is 1:
        ...
        if position is 1:
            print "Checked the routes: %s (use of 1 bus and 2 buses respectively). Best route is %s." % (str(combo_ID), str(combo_ID[position]))
        else:
            print "Checked the routes: %s (use of 1 bus and 2 buses respectively). Best combination is %s." % (str(combo_ID), str(combo_ID[position]))
            print combo_ID[position]
...
def hill_climbing(requestsList, feasible_routes, feas_index):
    ...
    while local_best is not current_best:
        ...
        for item in reqs:
            ...
            if with_two[0]:    
                ...
                combo_ID = with_two[3]
        if feasible_routes[len(feasible_routes)-1].number_of_buses is 2:
            # ELIMINATE COMBOS:
            ...
            SWAP = feasible_routes[len(feasible_routes)-1].combo_ID
    ...
    return feasible_routes, km_local_best, delay_local_best, SWAP
